//
// Created by Timana on 31/08/2026.
//
#include <iostream>
using namespace std;
...
struct Nodo {
    ...
    T dato2;
    Nodo<T>* sig;
    Nodo(T val, T dato2) : dato(val), sig(nullptr), dato2(dato2) {}
};
...
struct  ListaSimple {
    ...
    // ---------- INSERCIONES ----------
    void insertarInicio(T val, T dato2) {
        Nodo<T>* nuevo = new Nodo<T>(val, dato2);
        nuevo->sig = cabeza;
        cabeza = nuevo;
        if (tam == 0) cola = nuevo;
        tam++;
    ...
    void insertarFinal(T val, T dato2) {
        Nodo<T>* nuevo = new Nodo<T>(val,dato2);
        if (tam == 0) {
            cabeza = cola = nuevo;
        } else {
    ...
    bool insertarEnPosicion(int pos, T val, T dato2) {
        if (pos < 0 || pos > tam) return false;
        if (pos == 0) { insertarInicio(val, dato2); return true; }
        if (pos == tam) { insertarFinal(val, dato2); return true; }
...
ListaSimple<int> sumar_polinomios(ListaSimple<int>* Lista1, ListaSimple<int>* Lista2) {
    ListaSimple<int> Lista3;
    Nodo<int>* actual = Lista1->cabeza;
    Nodo<int>* actual2 = Lista2->cabeza;
    while (actual != nullptr && actual2 != nullptr) {
        if (actual->dato2 == actual2->dato2) {
            if (actual->dato + actual2->dato != 0) {
                Lista3.insertarFinal(actual->dato + actual2->dato, actual->dato2);
            }
            actual = actual->sig;
            actual2 = actual2->sig;
        }
        else if (actual->dato2 < actual2->dato2) {
            Lista3.insertarFinal(actual->dato, actual->dato2);
            actual = actual->sig;
        }
        else  {
            Lista3.insertarFinal(actual2->dato, actual2->dato2);
            actual2 = actual2->sig;
        }
    }
...
int main() {
    ...
    while (n--) {
        int val; int exp;
        cin >> exp >> val;
        polinomio1.insertarFinal(val, exp);
    }

    ListaSimple<int> polinomio2;
    ...
    while (n2--) {
        int val; int exp;
        cin >> exp >> val;
        polinomio2.insertarFinal(val, exp);
    }

    ListaSimple<int> polinomio3 = sumar_polinomios(& polinomio1, & polinomio2);
    polinomio3.imprimir();
    return 0;
}